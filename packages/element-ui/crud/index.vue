...
export default create({
  ...
  created () {
    this.setLoading = this.tableLoading
    // 初始化数据
    this.dataInit();
    // 规则初始化
    this.rulesInit();
  ...
  computed: {
    ...
    rowKey () {
      return this.tableOption.rowKey || "GUID_MAIN";
    },
    tableHeight () {
  ...
  methods: {
    ...
    dataInit () {
      ...
      this.list.forEach(function (ele, index) {
        for(let item in ele){//修改表格内日期为0000-00-00
          let is00=ele[item]==='0000-00-00'?true:false;
          if(is00){
            let index=_this.findColumnIndex(item)
            if(_this.columnOption[index] && _this.columnOption[index].type==='date'){
              ele[item]='';
            }
          }
        }
    ...
    rowDel (row, index) {
      // lsc: 回调返回增加option
      const tableData = {
        tableOption: this.deepClone(this.tableOption), 
        tableForm: this.deepClone(row)
      }
      this.$emit("row-del", tableData, index);
          ...
          if (index === 0) {
            ...
            switch (currItem.type) {
              ...
              case "sum":
                ...
                break;
              case "int":
                let intvals = data.map(item => Number(item[column.property]));
                sums[index] = intvals.reduce((perv, curr) => {
                  let intval = Number(curr);
                  if (!isNaN(intval)) {
                    return perv + curr;
                  } else {
                    return perv;
  ...
  updated() {
    this.$nextTick(function () {
      if(this.data.length>0) {
        setTimeout(() => {
          if(this.setLoading) this.setLoading = !this.setLoading
        }, 0);
      }
    })
